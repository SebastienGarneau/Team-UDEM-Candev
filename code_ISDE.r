...
# Print the pie chart
fn.graph.pie(nmbr.vec=nmbr.vec.gen, labels=labels.gen, group.name="Gender diversification",
           subtitle = "")
...
ind.perc <- round(ind.sum*100/(nmbr.non.ind + sum(nmbr.vec.ind)),1)

# Print the pie chart
fn.graph.pie(nmbr.vec=nmbr.vec.ind, labels=labels.ind, group.name="Percentages among indigenous people",
...
min.perc <- round(min.sum*100/(nmbr.non.min + sum(nmbr.vec.min)),1)

# Print the pie chart
fn.graph.pie(nmbr.vec=nmbr.vec.min, labels=labels.min, group.name="Percentages among minorities",
...
dis.perc <- round(dis.sum*100/(nmbr.non.dis + sum(nmbr.vec.dis)),1)

# Print the pie chart
fn.graph.pie(nmbr.vec=nmbr.vec.dis, labels=labels.dis, group.name="Percentages among people with disabilities",
...
df.total <- rbind(df.female, df.ind, df.dis, df.min)
...
legend('bottom',legend = c("Gender", "Race", "Indigenous", "Disability"), 
       col = 1:4, 
...
graph.CI(table = table.dis.2020, title = "Persons with disabilities")
graph.CI(table = table.ind.2020, title = "Indigenous peoples")
graph.CI(table = table.sex.2020, title = "Women")
graph.CI(table = table.race.2020, title = "Racialized persons")
...
########## PAS SUR QU<ON UTILISE CA



# Function that check which factors are the most problematic in each
# of our 4 groups, gender, race, indigenous people and diasbilities
#important.factors <- function(alpha, table.sex, table.ind, table.race, table.dis){
#  
#  Nbr.factor.signif <- rep(0,23)
#  
#  for(i in 1:23){
#    if(name.factor[i]%in%table.sex[which(table.sex[,4] < alpha),1]){
#      Nbr.factor.signif[i] <- Nbr.factor.signif[i] + 1
#    }
#    if(name.factor[i]%in%table.ind[which(table.ind[,4] < alpha),1]){
#      Nbr.factor.signif[i] <- Nbr.factor.signif[i] + 1
#   }
#    if(name.factor[i]%in%table.race[which(table.race[,4] < alpha),1]){
#      Nbr.factor.signif[i] <- Nbr.factor.signif[i] + 1
#    }
#    if(name.factor[i]%in%table.dis[which(table.dis[,4] < alpha),1]){
#      Nbr.factor.signif[i] <- Nbr.factor.signif[i] + 1
#    }
#  }
#  
#  return(Nbr.factor.signif)
#}

#important.factors(alpha = 0.05, table.sex.2020, table.ind.2020, table.race.2020, table.dis.2020)
#important.factors(alpha = 0.01, table.sex.2020, table.ind.2020, table.race.2020, table.dis.2020)
#important.factors(alpha = 0.001, table.sex.2020, table.ind.2020, table.race.2020, table.dis.2020)

# The 3 factors that are prevalent in 2020
#name.factor[1]
#name.factor[6]
#name.factor[20]

#important.factors(alpha = 0.05, table.sex.2020, table.ind.2020, table.race.2020, table.dis.2020)
#important.factors(alpha = 0.05, table.sex.2019, table.ind.2019, table.race.2019, table.dis.2019)
#important.factors(alpha = 0.05, table.sex.2018, table.ind.2018, table.race.2018, table.dis.2018)
